...
class AamarPay {
  ...
  async initiatePayment(paymentData) {
    ...
    const payload = {
      ...
      return_url:
        returnUrl || `${process.env.AAMARPAY_CALLBACK_URL}?order_id=${orderId}`,
      cancel_url:
        cancelUrl ||
        `${process.env.AAMARPAY_CALLBACK_URL}?status=cancelled&order_id=${orderId}`,
  ...
  // CRITICAL: Always use the 'Search' API for final verification
  async verifyPayment(transactionId) {
    const params = new URLSearchParams({
      store_id: this.storeId,
      signature_key: this.signatureKey,
      type: "json", // Important: many versions require this for JSON response
      request_id: transactionId, // Sometimes 'trx_id' or 'request_id' depending on API version
    });

    try {
      const response = await fetch(
        `${this.baseUrl}/api/v1/trxcheck.php?${params.toString()}`,
      );
      const data = await response.json();

      // Check for "Valid" status from the ACTUAL server response
      return data.pay_status === "Successful";
    } catch (error) {
      return false;
    }
  }
